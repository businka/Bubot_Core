...
from bubot.coap import Message, NON, Code
from .DeviceLink import ResourceLink
import urllib.parse
import cbor2
...
class OcfRequest(OcfMessage):
    ...
    def decode_from_coap(cls, msg, multicast=False):
        # msg = Message.decode(raw_data, remote)
        self = cls(
            fr=ResourceLink.init_from_uri('coap://[{0}]:{1}'.format(msg.remote[0], msg.remote[1])),  # msg.opt.uri_path, msg.opt.uri_port))]
...
class OcfResponse(OcfMessage):
    ...
    def encode_to_coap(self):
        ...
        # msg = self.raw_msg
        # msg.code = self.code
        # msg.payload = cbor2.dumps(self.data) if self.data else b''
        # msg.mtype = NON
